...
export default function App() {
  ...
  const text = "I build visually appealing websites that deliver a great user experience";
  const finalText = "I would love the opportunity to join a team of developers where I can learn from the best, gaining insights into sophisticated web development and site-building techniques";
  ...
  gsap.fromTo(description_1.current,
   { y: "60vh" },
   {
    scrollTrigger: description_1.current, // start animation when ".box" enters the viewport
    y: 0
   }
  );

  gsap.fromTo(description_2.current,
    { y: "70vh" },
    {
     scrollTrigger: description_2.current, // start animation when ".box" enters the viewport
     y: 0
    }
   );

   gsap.fromTo(stack.current,
    { scale: 0 },
    {
     scrollTrigger: stack.current, // start animation when ".box" enters the viewport
     scale: 1,
     delay: 1,
    }
   );

   gsap.fromTo(".stack-tech",
    { scale: 0 },
    {
     scrollTrigger: stackTech.current, // start animation when ".box" enters the viewport
     scale: 1,
     delay: 1,
     stagger: 0.2,
    }
   ...
   gsap.utils.toArray(".picture-gallery").forEach((item) => {
    gsap.fromTo(item,
      { y: "100%", opacity: 0 },
      {
        ...
        scrollTrigger: {
          trigger: item,
          start: "top 80%", // Inizia quando l'elemento entra nell'80% della viewport
          toggleActions: "play none none none", // Esegui una sola volta
          onEnter: () => {
            // Aggiungo il rimbalzo dopo il primo movimento
            gsap.to(item, {
              y: 0,
              duration: 1 / 2,
  ...
  return (
    <>
      ...
      <div className="min-h-[100vh] py-10 bg-[#1a1a1a] text-white flex flex-col justify-center items-center px-[2rem] uppercase font-500 mx-auto">
        <div className="w-[80%] h-[100%] mx-auto flex flex-col items-center justify-center gap-[3rem]">
          <p ref={description_1} className="text-2xl md:text-4xl text-[#80d4ff]">I'm a front end developer eager to join a company in order to learn the job from experts.</p>
          <p ref={description_2} className="text-xl md:text-3xl">I use simple yet effective colors to create the structure and organization of the site.
          I always strive to improve the final result I deliver to users and pay close attention to how the site is represented.</p>
        </div>
      </div>
      <div className="w-[100vw] min-h-[100vh] md:min-h-[100vh] bg-[#1a1a1a] px-[2rem] flex flex-col justify-center items-center">
        <p ref={stack} className="text-4xl md:text-7xl text-amber-300 uppercase font-bold">stack</p>
        <div ref={stackTech} className="mt-[4rem] grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-[2rem]">
          <p className="stack-tech text-amber-100 uppercase  rounded-[10px] px-2 md:rounded-4xl border-amber-300 border-[2px] w-[100%] md:w-[150px] my-[1rem] text-center py-1">html</p>
          <p className="stack-tech text-amber-100 uppercase  rounded-[10px] px-2 md:rounded-4xl border-amber-300 border-[2px] w-[100%] md:w-[150px] my-[1rem] text-center py-1">css</p>
          <p className="stack-tech text-amber-100 uppercase  rounded-[10px] px-2 md:rounded-4xl border-amber-300 border-[2px] w-[100%] md:w-[150px] my-[1rem] text-center py-1">javascript</p>
          <p className="stack-tech text-amber-100 uppercase  rounded-[10px] px-2 md:rounded-4xl border-amber-300 border-[2px] w-[100%] md:w-[150px] my-[1rem] text-center py-1">react</p>
          <p className="stack-tech text-amber-100 uppercase  rounded-[10px] px-2 md:rounded-4xl border-amber-300 border-[2px] w-[100%] md:w-[150px] my-[1rem] text-center py-1">react router</p>
          <p className="stack-tech text-amber-100 uppercase  rounded-[10px] px-2 md:rounded-4xl border-amber-300 border-[2px] w-[100%] md:w-[150px] my-[1rem] text-center py-1">redux toolkit</p>
          <p className="stack-tech text-amber-100 uppercase  rounded-[10px] px-2 md:rounded-4xl border-amber-300 border-[2px] w-[100%] md:w-[150px] my-[1rem] text-center py-1 col-start-2">tailwind css</p>
        </div>
      </div>

      <div  className="min-h-[100vh] projects py-[4rem] bg-[#1a1a1a] px-[2rem] md:px-8 flex flex-col justify-center items-center">
      ...
      {char}
        ...
        <div className="w-[90%] md:w-[80%] mx-auto my-[4rem]">
          {projects.map((item,index) => {
            return (
              <div key={index} className="item-gallery text-center flex flex-col md:text-left md:flex-row justify-between my-[4rem] gap-x-2">
              <div className=" text-white flex-2/3 flex flex-col gap-4" key={index}>
                <p className="text-4xl uppercase font-bold text-outline-yellow my-5">{item.title}</p>
                ...
                <p className="w-[100%] lg:w-[80%] my-8 md:my-4 lg:my-0 text-center md:text-left">{item.description}</p>
              </div>
              <div  className="flex-1/3 flex justify-center md:items-center picture-gallery"> 
                <a href={item.url} target="_blank">
                  <img src={item.img} alt={item.title} className="w-[300px] h-[150px]" />
                </a>
              </div>
              </div> 
            )
      ...
      <div className="bg-[#1a1a1a] px-8 flex flex-col gap-22">
        <div className="w-[90%] md:w-[80%] mx-auto">
          ...
          {finalText.split(" ").map((item,index) => {
            return (
              <>
                <span className="inline-block final-text" key={index}>{item}</span>
                <span> </span>
              </>
            )
        ...
        <p className="text-[#80d4ff] text-[1rem] text-center md:text-left md:text-4xl font-bold uppercase email">giovanni.serra.web@gmail.com</p>
        </div>
        <div className="flex md:flex-1/3 justify-center md:justify-end items-end gap-4 md:gap-8 my-10">
        <a href="https://github.com/Giovanni-Serra-Git" target="_blank" className="inline-block icon">
          <img src="/logo/github.svg" alt="Github Logo" width="50" className=" transition-transform duration-300 hover:scale-150 ease-in-out"/>
        </a>
        <a href="https://www.linkedin.com/in/giovanni-serra-609a84277/" target="_blank" className="inline-block icon">
          <img src="/logo/linkedin.svg" alt="Linkedin Logo" width="50" className=" transition-transform duration-300 hover:scale-150 ease-in-out"/>
        </a>
        </div>
        </div>
      </div>
